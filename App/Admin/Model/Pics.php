...
class Pics extends Model{
	...
	public function remove(){
		$pics_id = Q('pics_id');
		//获取图集数据，并删除文件
		$pic = $this->find($pics_id);
		is_file($pic['pics_big']) and unlink($pic['pics_big']);
		is_file($pic['pics_mid']) and unlink($pic['pics_mid']);
		is_file($pic['pics_small']) and unlink($pic['pics_small']);

		//删除数据库记录
		return $this->where('pics_id',$pics_id)->delete();
	}

	//获取单挑图集信息
	public function getAll(){
		return $this->where('goods_id',Q('goods_id'))->get();
	}
